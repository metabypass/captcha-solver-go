...
func serviceGetAccessToken() (string, bool) {
	...
	payload := fmt.Sprintf(`{"grant_type":"password","client_id": "%s" ,"client_secret": "%s","username": "%s","password": "%s"}`, CLIENT_ID, CLIENT_SECRET, EMAIL, PASSWORD)
	newPayload := strings.NewReader(payload)
...
func recaptchaV3(sitekey string, siteUrl string) (string, int, string) {
	payloadString := fmt.Sprintf(`{
	"sitekey":"%s",
	  "version": 3 ,
	  "url": "%s"
...
func recaptchaV2(sitekey string, siteUrl string) (string, int, string) {
	payloadString := fmt.Sprintf(`{
	"sitekey":"%s",
	  "version": 2 ,
	  "url": "%s"
